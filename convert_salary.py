...
def convert_salary(input_file, cur_history_file, output_file):
    ...
    vacancies = vacancies[['name', 'salary', 'area_name', 'published_at']]
    print('Dataframe ready')

    vacancies.head(100).to_csv(output_file, index=False)
    print('CSV file created')


def main():
    convert_salary('csv/vacancies_dif_currencies.csv', 'csv/currensies_history.csv', 'output.csv')
